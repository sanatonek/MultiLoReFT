...
def generate_multimodal_data(n_samples, mod_dim, save_path):
    ...
    # rather uniform sampling
    #W_m1 = np.random.uniform(-1.0, 1.0, (5, mod_dim)) * 0.2
    #W_m2 = np.random.uniform(-1.0, 1.0, (6, mod_dim)) * 0.1

    X_1 = np.concat([X_m1, X_s], -1)
    X_2 = np.concat([X_m2, 2*X_s], -1)
...
if __name__ == "__main__":
    n_samples = 6000
    mod_dim = 5
    save_path = "./data/simulated_data.npz"
    # set a random seed for reproducibility
    np.random.seed(0)
    torch.manual_seed(0)
    import random
    random.seed(0)
    h1, h2, x1, x2, labels = generate_multimodal_data(n_samples, mod_dim, save_path)
