...
class ScaledLoss:
    """Scaled Loss for balancing multiple losses."""
    def __init__(self, scales: list):
        self.scales = scales
    
    def __call__(self, losses, model, trainable_params):
        losses = torch.stack([l if isinstance(l, torch.Tensor) else torch.tensor(l) for l in losses])
        return sum(scale * loss for scale, loss in zip(self.scales, losses)), self.scales
...
def loss_orthogonality(R_s, R_m1, R_m2):
    ...
    """
    # this only enforces orthogonality between row spaces
    loss_ortho_1 = torch.norm(torch.mm(R_s, R_m1.T), p="fro")**2
    loss_ortho_2 = torch.norm(torch.mm(R_m1, R_m2.T), p="fro")**2 # enforcing the row spaces to be orthogonal
    loss_ortho_3 = torch.norm(torch.mm(R_s, R_m2.T), p="fro")**2
    return loss_ortho_1 + loss_ortho_2 + loss_ortho_3

    # now we are enforcing orthogonality between the row and column spaces
    #loss_ortho_1 = torch.norm(torch.mm(R_s, R_m1.T), p="fro")**2 + torch.norm(torch.mm(R_s.T, R_m1), p="fro")**2
    #loss_ortho_2 = torch.norm(torch.mm(R_s, R_m2.T), p="fro")**2 + torch.norm(torch.mm(R_s.T, R_m2), p="fro")**2
    #loss_ortho_3 = torch.norm(torch.mm(R_m1, R_m2.T), p="fro")**2 + torch.norm(torch.mm(R_m1.T, R_m2), p="fro")**2
    #return loss_ortho_1 + loss_ortho_2 + loss_ortho_3
...
#def compute_stage_losses(model, h1, h2, z_components, stage, phis):
def compute_stage_losses(model, h1, h2, z_components, stage, phis=None):
    """
    ...
    if model.mi_loss:
        ...
        # mse losses
        l_mi = torch.nn.functional.mse_loss(phis[0], h1) + torch.nn.functional.mse_loss(phis[1], h2)
    ...
    if model.crosscov_loss:
        l_cov = loss_cross_covariance(z_components[0][1], z_components[1][1], z_components[0][0], z_components[1][0])
        all_losses.append(l_cov.item())
        all_loss_names.append("Cross Covariance Loss")
        ...
        if stage == "shared":
            ...
            return [l_orthogonal, l_shared, l_mi, l_cov], ["Orthogonal Loss", "Shared Loss", "Mutual Info Loss", "Cross Covariance Loss"], all_losses, all_loss_names
        else:
