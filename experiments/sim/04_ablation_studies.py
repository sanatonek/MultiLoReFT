...
def train(
    ...
    if loss_balance == 'gradient':
        ...
        loss_balancer = ScaledLoss(scales=[1.0, 1.0, 1.0])
    all_epoch_losses = []
    lrs = []
    all_epoch_stages = []
    ...
    for epoch in range(epochs):
        ...
        if epoch == 0:
            if not joint:
                ...
                print(f"[Epoch {epoch}] → Starting with JOINT stage.")
...
def main(dev_id=0, out_dir="./results/", file_name="sweep", sim_data='sim_100000_in5-5_data5-5_shared2_c2_shared', subset=10000):
    """Main function to run the training pipeline."""
    # add the seed to the file name
    file_name = f"{file_name}_{sim_data}_subset{subset}"
...
if __name__ == "__main__":
    ...
    main(dev_id=args.gpu, out_dir=args.out_dir, file_name=args.file_name, sim_data=args.data, subset=args.subset)