...
from src.model import MultiLoReFT, MM_Adapter
                    ...
                    for lr_anneal in hyperparameters["lr_schedule"]:
                        for staging in hyperparameters["staging"]:
                            for pruning in hyperparameters["pruning"]:
                                ...
                                h1 = torch.Tensor(val_dataloader.dataset.h1).float().to(device)
                                h2 = torch.Tensor(val_dataloader.dataset.h2).float().to(device)
