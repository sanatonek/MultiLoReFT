...
def evaluate_regression(z_n, h1, h2):
    ...
    for i in range(2):
        for j in range(3):
            ...
            early_stopping = 10
            val_losses = []
            #reg_pbar = tqdm(range(500), desc="Training linear regression", leave=True)
            ...
            #print(f'Zs{i}-Zm{i} -----Goodness of fit (R2 score): {r_squares.mean():.3f} (var: {r_squares.std():.3f})')
            if j == 0:
                name = "Zm"
            elif j == 2:
                name = "(Zm+Zs)"
...
def eval_model(model, test_h1, test_h2, test_labels, device, threshold=0.05):
    ...
    # add the true ranks of the subspaces
    # Get matrices above threshold
    shared_sv = len(list(torch.where(torch.linalg.svdvals(model.R_s) > threshold)))
    m1_sv = len(list(torch.where(torch.linalg.svdvals(model.R_m1) > threshold)))
    m2_sv = len(list(torch.where(torch.linalg.svdvals(model.R_m2) > threshold)))
    rank_df = pd.DataFrame({
        "name": ["R_s", "R_m1", "R_m2"],
        "rank": [shared_sv, m1_sv, m2_sv]
    })
